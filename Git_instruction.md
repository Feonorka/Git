...
В случае если необходимо поработать с чужим репозиторием и перенести его на свой аккаунт на GitHub есть функция **fork**. Далее уже можно данный *удаленный репозиторий* с помощью команды `git clone` перенести и сделать локальным, создать ветку `git branch` (работаем только в ней) в ней поработать с ним. В таких ситуациях принято создавать файл **README** в репозитории. В этом файле дается описание репозитория.

После этого с помощью команды `git push` залить изменения в свой акаунт на GitHub.
Таким образом с помощью функции **pull request** можно предложить исходному автору свои изменения в репозиторий.